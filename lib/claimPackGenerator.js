...
- Cite evidence IDs (8-char format) and dates for every claim
- Do NOT speculate or add generic statements
- If a systematic step is missing, explicitly state the gap
- Use clear, professional language (NO hashtags or social media formatting)

Output structured format:

### Technical Uncertainty
[Why the outcome wasn't knowable upfront - cite evidence - 2-3 sentences]

### Hypothesis
[Testable technical proposition with success criteria from evidence - 1-2 sentences]

### Experimental Method
[How tested: methods, metrics, test environments - cite evidence - 2-3 sentences]

### Systematic Work & Results
[Chronological progression citing evidence IDs and dates - 3-5 sentences]
${missingSteps.length > 0 ? `[⚠ MISSING STEPS: ${missingSteps.join(', ')} - flag this gap clearly]` : ''}

### Conclusion & New Knowledge
[What was learned from evidence - 2-3 sentences]

Total: 400-600 words in plain text with markdown headings.
Every statement must be traceable to specific evidence.`;
}
...
export async function generateClaimPackSection(sectionKey, projectData) {
  const { project, activities, evidence, narratives, costLedger, knowledgeDocs = [] } = projectData;
