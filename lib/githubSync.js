 ...
 * @returns {boolean} True if any keyword found, or if keywords array is empty
 */
 ...
 * @param {string[]} [options.branches] - Optional branch names to filter
 * @returns {Promise<Array>} Array of commit objects
 */
export async function fetchGitHubCommits({ owner, repo, since, token, branches }) {
  ...
  // If branches specified, fetch from each branch and deduplicate
  if (branches && branches.length > 0) {
    console.log(`[githubSync] Fetching commits from ${branches.length} branch(es): ${branches.join(', ')}`);
    ...
    for (const branch of branches) {
      ...
      console.log(`[githubSync] Fetching commits from branch "${branch}": ${url}`);

      const response = await fetch(url, { headers });

      if (!response.ok) {
        const error = await response.text();
        // If branch doesn't exist, log and skip rather than failing
        if (response.status === 422 || response.status === 404) {
          console.warn(`[githubSync] Branch "${branch}" not found, skipping`);
          continue;
        }
        console.error(`[githubSync] GitHub API error for branch "${branch}": ${response.status} - ${error}`);
        throw new Error(`GitHub API error: ${response.status} - ${error}`);
      }

      const commits = await response.json();
      console.log(`[githubSync] Branch "${branch}": ${commits.length} commits`);
    ...
    console.log(`[githubSync] Total unique commits across branches: ${allCommits.length}`);
    return allCommits;
  }

  // Default: fetch from default branch (current behavior)
  const params = new URLSearchParams();
  if (since) {
    params.append('since', new Date(since).toISOString());
  }
 ...
 * @param {string} options.since - ISO date string or null
 * @param {string} options.token - GitHub access token
 * @param {string[]} [options.branches] - Optional branch names to filter (matches base or head branch)
 * @returns {Promise<Array>} Array of pull request objects
 */
export async function fetchGitHubPullRequests({ owner, repo, since, token, branches }) {
  const params = new URLSearchParams();
  ...
  // Filter by branches if specified (match base or head branch)
  if (branches && branches.length > 0) {
    ...
    filtered = filtered.filter(pr => {
      ...
      return branchSet.has(baseBranch) || branchSet.has(headBranch);
    });
    console.log(`[githubSync] Branch-filtered PRs: ${beforeCount} → ${filtered.length}`);
  }

  console.log(`[githubSync] Received ${prs.length} PRs, ${filtered.length} after filters`);
...
/**
 * Main sync function - syncs commits AND pull requests with optional filters
 * @param {string} projectId - Project UUID
 * @param {string} repoOwner - Repository owner
 * @param {string} repoName - Repository name
 * @param {string} accessToken - GitHub access token
 * @param {Object} [filters] - Optional filters
 * @param {string[]} [filters.filterBranches] - Branch names to fetch from
 * @param {string[]} [filters.filterKeywords] - Keywords to match in commit messages/PR titles
 * @returns {Promise<Object>} Sync results {synced: number, skipped: number, reasons: Object}
 */
export async function syncGitHubData(projectId, repoOwner, repoName, accessToken, { filterBranches, filterKeywords } = {}) {
  console.log(`[githubSync] Starting sync for ${repoOwner}/${repoName}, project: ${projectId}`);
  if (filterBranches?.length) console.log(`[githubSync] Branch filter: ${filterBranches.join(', ')}`);
  if (filterKeywords?.length) console.log(`[githubSync] Keyword filter: ${filterKeywords.join(', ')}`);
  ...
  const commits = await fetchGitHubCommits({
    ...
    since,
    token: accessToken,
    branches: filterBranches?.length ? filterBranches : undefined
  });

  console.log(`[githubSync] Fetched ${commits.length} commits from GitHub`);

  if (commits.length > 0) {
    // Apply keyword filter before R&D filter
    let keywordFiltered = commits;
    if (filterKeywords?.length > 0) {
      const beforeCount = commits.length;
      keywordFiltered = commits.filter(commit => {
        const message = commit.commit.message;
        return matchesKeywords(message, filterKeywords);
      });
      const dropped = beforeCount - keywordFiltered.length;
      if (dropped > 0) {
        allReasons.keyword_filtered = dropped;
        console.log(`[githubSync] Keyword filter removed ${dropped} commits (kept ${keywordFiltered.length})`);
      }
    }

    const { filtered: filteredCommits, skipReasons: commitReasons } = preFilterCommits(keywordFiltered);
  ...
  if (prs.length > 0) {
    // Apply keyword filter to PRs
    let keywordFiltered = prs;
    if (filterKeywords?.length > 0) {
      const beforeCount = prs.length;
      keywordFiltered = prs.filter(pr => {
        const combined = `${pr.title || ''} ${pr.body || ''}`;
        return matchesKeywords(combined, filterKeywords);
      });
      const dropped = beforeCount - keywordFiltered.length;
      if (dropped > 0) {
        allReasons.pr_keyword_filtered = dropped;
        console.log(`[githubSync] Keyword filter removed ${dropped} PRs (kept ${keywordFiltered.length})`);
      }
    }

    const { filtered: filteredPRs, skipReasons: prReasons } = preFilterPullRequests(keywordFiltered);
