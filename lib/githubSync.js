...
export function preFilterCommits(commits, participants) {
  ...
  const filtered = commits.filter(commit => {
    ...
    if (!isParticipant) {
      skipReasons.not_participant++;
      console.log(`[githubSync] Skipping commit (not participant): "${message.substring(0, 60)}..." by ${authorEmail} (participants: ${participants.join(', ')})`);
      return false;
    }
...
export async function syncCommits(projectId, repoOwner, repoName, accessToken, participants) {
  ...
  console.log(`[githubSync] Participants:`, participants);
  ...
  const { filtered, skipReasons } = preFilterCommits(commits, participants);
