...
/**
 * Gets a valid Xero access token for a user, refreshing if needed.
 * Returns { accessToken, tenantId, tenantName, error }
 */
export async function getXeroToken(userId) {
  if (!userId) {
    return { accessToken: null, tenantId: null, tenantName: null, error: 'No user ID provided' };
  }
  ...
  if (!tokenRow) {
    return { accessToken: null, tenantId: null, tenantName: null, error: 'Xero not connected. Please connect Xero first.' };
  }
  ...
  // Token expired or expiring soon — refresh it (Xero tokens last 30 min)
  const clientId = process.env.XERO_CLIENT_ID;
  const clientSecret = process.env.XERO_CLIENT_SECRET;

  if (!clientId || !clientSecret) {
    return { accessToken: null, tenantId: null, tenantName: null, error: 'Xero OAuth not configured on server' };
  }
  ...
  try {
    ...
    if (!refreshResponse.ok) {
      const errText = await refreshResponse.text();
      console.error('[Xero Auth] Token refresh failed:', refreshResponse.status, errText);
      return { accessToken: null, tenantId: null, tenantName: null, error: 'Xero token refresh failed. Please reconnect Xero.' };
    }

    const refreshData = await refreshResponse.json();
    const newExpiresAt = new Date(Date.now() + (refreshData.expires_in || 1800) * 1000).toISOString();
    ...
    console.error('[Xero Auth] Token refresh error:', err);
    return { accessToken: null, tenantId: null, tenantName: null, error: 'Failed to refresh Xero token' };
  }
}
