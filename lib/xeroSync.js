...
export async function fetchBills(accessToken, tenantId, startDate, endDate) {
  const whereClause = `Type=="ACCPAY" AND Date>="${startDate}" AND Date<="${endDate}"`;
  const url = `${XERO_ACCOUNTING_BASE}/Invoices?where=${encodeURIComponent(whereClause)}&Statuses=AUTHORISED,PAID&page=1`;
